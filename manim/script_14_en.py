"""Article 14 — Synthetic Aperture Radar (EN). Animation steps:

1. Show a small SAR antenna on the left + a target ground point
2. Antenna emits a chirp toward the ground (pulse + echo)
3. Antenna moves right and emits again (3 positions total)
4. A long horizontal bar appears between the 3 positions = "synthesized aperture"
5. Show that the resulting beam resolution is much finer than the small antenna alone
"""
from manim import (
...
class RadarArticle14(Scene):
    def construct(self):
        ...
        title = Text("Synthetic Aperture Radar",
        ...
        step = Text("1. PULSE  --  chirp toward target",
        ...
        # ---- 2. MOVE ----
        step2 = Text("2. MOVE  --  shift 2 meters, pulse again",
        ...
        # ---- 3. MOVE AGAIN ----
        step3 = Text("3. MOVE AGAIN  --  pulse from position 3",
                     color=YELLOW, font_size=15).to_edge(DOWN)
        ...
        # ---- 4. COMBINE  --  synthesized aperture ----
        step4 = Text("4. COMBINE  --  all echoes = synthesized aperture",
        ...
        synth_label = Text("synthesized aperture ~ 3 m",
        ...
        # Fade old pulses/echoes; show how this gives a sharper image
        all_pulses = VGroup(pulse, pulse2, pulse3, echo, echo2, echo3)
        self.play(FadeOut(all_pulses), run_time=0.6)

        final = Text("small antenna + motion = huge synthetic aperture",
